...
var jobList = [];
var hostList;
export var serviceList = ["Temperature", "Job_load", "Memory_usage", "Fans_speed", "Power_consum", "Job_scheduling"];
...
export var serviceListattrnest = [
...
function onClusterHistogram() {
    ...
    let violiin_chart = d3.histChart().graphicopt({
        width: w, height: h, opt: { dataformated: true }, tick: { visibile: false },
    ...
    d3.select('#clusterDisplay').selectAll('.radarCluster').append('svg')
        ...
        .each(function (d) {
            violiin_chart.graphicopt({
                color: (i) => colorCluster(d.id), title: [{ text: `${nestCluster[d.id].total} state${nestCluster[d.id].total > 1 ? 's' : ''}` }
                    , { text: `${nestCluster[d.id].extra} node${nestCluster[d.id].total > 1 ? 's' : ''}` }]
            }).rangeY(customrangeY).data([nestCluster[d.id]]).draw(d3.select(this))
        });
